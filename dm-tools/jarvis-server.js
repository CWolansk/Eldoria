 ...
 *   2. npm install
 *   3. npm run start:jarvis
 *   4. Open http://localhost:8087/jarvis in Chrome/Edge.
 */
...
const DM_TOOLS_DIR = __dirname;
const VAULT_ROOT = path.resolve(DM_TOOLS_DIR, '..');
const PUBLIC_DOCS_DIR = path.join(VAULT_ROOT, 'docs');
...
const VAULT_INDEX = path.join(DM_TOOLS_DIR, 'data', 'jarvis-index.json');
...
try {
    ...
    console.warn('⚠  data/jarvis-index.json not found — run: npm run gen:jarvis');
}
...
// ---------------------------------------------------------------------------
// Static file server for local-only DM tools.
// ---------------------------------------------------------------------------
const MIME = {
...
const httpServer = http.createServer((req, res) => {
    ...
    if (urlPath === '/' || urlPath === '/jarvis') urlPath = '/tools/dm-jarvis.html';
    if (urlPath === '/item-editor') urlPath = '/tools/item-editor.html';
...
function resolveStaticPath(urlPath) {
    let root = DM_TOOLS_DIR;
    let relPath = urlPath.replace(/^\/+/, '');

    if (urlPath.startsWith('/docs/')) {
        root = PUBLIC_DOCS_DIR;
        relPath = urlPath.slice('/docs/'.length);
    }
...
function attemptRequire(mod) {
    try {
        ...
        console.error(`    Run: cd dm-tools && npm install\n`);
        process.exit(1);
    }
}
